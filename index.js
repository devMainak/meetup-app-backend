...
app.post("/meetups", async (req, res) => {
  try {
    ...
    if (!savedMeetUp) {
      res.status(400).json({ error: "Failed to save meetup data." });
    } else {
...
app.get("/meetups", async (req, res) => {
  try {
    ...
    if (allMeetUps.length != 0) {
      res.status(200).send(allMeetUps);
    } else {
      res.status(400).json({ error: "Failed to fetch meetups" });
    }
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error." });
...
app.get("/meetups/:meetUpId", async (req, res) => {
  try {
    ...
    if (!meetUpById) {
      res.status(404).json({ error: "Meetup details not found." });
    } else {
...
app.post("/meetups/:meetUpId", async (req, res) => {
  try {
    ...
    if (!updatedMeetUp) {
      res.status(400).json({ error: "Meetup not found" });
    } else {
      res
        .status(201)
        .json({
